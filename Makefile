...
heroku-16:
	@echo "Running unit tests in docker (heroku-16)..."
	@docker run -v $(shell pwd):/buildpack:ro --rm -it -e "STACK=heroku-16" -e "BUILDPACK_HOME=/buildpack" heroku/heroku:16 bash -c 'cp -r /buildpack /buildpack_test; cd /buildpack_test/; test/unit;'
	@echo ""
	@echo "Running v2 integration tests in docker (heroku-18)..."
...
cedar-14:
	@echo "Running unit tests in docker (cedar-14)..."
	@docker run -v $(shell pwd):/buildpack:ro --rm -it -e "STACK=cedar-14" -e "BUILDPACK_HOME=/buildpack" heroku/cedar:14 bash -c 'cp -r /buildpack /buildpack_test; cd /buildpack_test/; test/unit;'
	@echo ""
	@echo "Running v2 integration tests in docker (heroku-18)..."
