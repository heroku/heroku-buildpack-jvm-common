...
testSecondaryEnvVar() {
    local old_ci="$CI"
    unset CI
    set_jdbc_url "postgres://foo:bar@ec2-0-0-0-0:5432/abc123" "SCALINGO_POSTGRESQL_RED_JDBC"
    assertEquals "Wrong secondary DB URL" \
                 "jdbc:postgresql://ec2-0-0-0-0:5432/abc123?user=foo&password=bar&sslmode=require" \
                 "$SCALINGO_POSTGRESQL_RED_JDBC_URL"
    unset SCALINGO_POSTGRESQL_RED_JDBC_URL
    export CI="$old_ci"
}
