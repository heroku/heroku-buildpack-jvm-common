...
describe "Java" do
  ...
  before(:each) do
    ...
    app.setup!
    app.heroku.set_config("JVM_COMMON_BUILDPACK", "https://github.com/heroku/heroku-buildpack-jvm-common/tarball/#{`git rev-parse HEAD`}")
  ...
  ["1.7", "1.8"].each do |version|
    context "on jdk-#{version}" do
      ...
      it "should not install settings.xml" do
        app.heroku.set_config("BUILDPACK_URL", "https://github.com/heroku/heroku-buildpack-java#upgrade-jvm-common")
        app.deploy do |app|
